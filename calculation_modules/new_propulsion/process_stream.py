...
from aircraft_modules.new_propulsion.components import GenericPropulsionComponent
from aircraft_modules.new_propulsion.components import ComponentStream
from aircraft_modules.new_propulsion.components import Booster, Combustor, Compressor, Fan
    ...
    def compute_stream(self, T_a, P_a):
        ...
        for stream in self.process_streams:
            ...
            for component_name in self.process_streams[stream]:
                ...
                self.set_thermodynamic_states(dict_T_0, component_name, temperature, stream)
                self.set_thermodynamic_states(dict_P_0, component_name, pressure, stream)
                
                if isinstance(process, CombustionProcess):
                    fuel_air_ratio = process.get_fuel_air_ratio()
                    self.set_thermodynamic_states(dict_fuel_air_ratio, component_name, fuel_air_ratio, stream)
                
                if isinstance(process, NozzleProcess):
                    outlet_speed = process.get_outlet_speed(temperature, pressure)
                    self.set_thermodynamic_states(dict_outlet_speed, component_name, outlet_speed, stream)
        
        return dict_T_0, dict_P_0, dict_outlet_speed, dict_fuel_air_ratio


    def set_thermodynamic_states(self, dict_thermodynamic_state, component_name, thermodynamic_state, stream_id):
