...
def main():
    ...
    ''' Analysis Parameters'''
    list_of_mach = [0, 0.85]
    list_of_Pa = [101.30, 18.75]
    list_of_Ta = [288.2, 216.7]
    N_2 = 1
    ...
    fan = new_propulsion_module.get_component(1, 'fan')
    ...
    for k, (mach, Pa, Ta) in enumerate(zip(list_of_mach, list_of_Pa, list_of_Ta)):
        axis1 = fig1.add_subplot(1,2,k+1)
        axis2 = fig2.add_subplot(1,2,k+1)
        axis3 = fig3.add_subplot(1,2,k+1)
        Po = Pa
        ...
        for combustor_outlet_temperature in list_of_combustor_outlet_temperature:
            ...
            axis1.plot(list_of_air_passage_ratio, list_of_specific_thrust, marker='o', label = combustor_outlet_temperature)
            axis1.set_ylim([0, 2600])
            axis1.set_title("m = {}, T_a = {}, P_a = {}".format(mach, Ta, Pa))
            axis1.legend()

            
            axis2.plot(list_of_air_passage_ratio, list_of_TSFC, marker='o', label = combustor_outlet_temperature)
            axis2.set_ylim([0, 0.00003])
            axis2.set_title("m = {}, T_a = {}, P_a = {}".format(mach, Ta, Pa))
            axis2.legend()
            
            axis3.plot(list_of_air_passage_ratio, list_of_thermal_efficiency, marker='o', label = combustor_outlet_temperature)
